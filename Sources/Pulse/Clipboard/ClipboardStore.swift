...
final class ClipboardStore: @unchecked Sendable {
    ...
    func loadAll(completion: @escaping @MainActor ([ClipItem]) -> Void) {
        queue.async { [self] in
            var items: [ClipItem] = []
            try? database?.query(
                "SELECT id, kind, text, image_file, width, height, hash, source, created FROM items ORDER BY created DESC LIMIT ?",
                [.int(Int64(maxItems))]
            ) { row in
    ...
    func record(_ capture: ClipCapture, sourceBundleID: String?, completion: @escaping @MainActor (ClipItem, [String]) -> Void) {
        queue.async { [self] in
            ...
            do {
                ...
                try database.query(
                    "SELECT id, kind, text, image_file, width, height, hash, source, created FROM items WHERE hash = ?",
                    [.text(hash)]
    ...
    func deleteAll() {
        queue.async { [self] in
            try? database?.run("DELETE FROM items")
    ...
    private func prune(_ database: SQLiteDatabase) throws -> [String] {
        ...
        try database.query(
            "SELECT id, image_file FROM items ORDER BY created DESC LIMIT -1 OFFSET ?",
            [.int(Int64(maxItems))]
        ) { row in
